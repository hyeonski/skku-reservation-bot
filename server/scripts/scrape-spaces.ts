...
(() => {
  ...
  function selectComboByText(dm, comboSuffix, label) {
    ...
    const items = Array.from(document.querySelectorAll('div[id*=".combolist.item_"]'))
      .filter(d => d.id.startsWith(itemPrefix) && !d.id.endsWith(':text'));
    const target = items.find(it => it.innerText.trim() === label);
    if (!target) {
      const avail = items.map(i => i.innerText.trim()).join(', ');
...
async function selectCampus(page: Page, campusName: string): Promise<void> {
  await page.evaluate(
    ({ suffix, name }) => {
      ...
      g.selectComboByText(dm, suffix, name);
    },
    { suffix: MODAL_FIELDS.캠퍼스, name: campusName },
  );
  await sleep(1500);
}
...
async function selectBuilding(page: Page, buildingName: string): Promise<void> {
  await page.evaluate(
    ({ suffix, name }) => {
      const g = (window as any).__gls;
      const dm = g.activeModalDM();
    ...
    { suffix: MODAL_FIELDS.건물, name: buildingName },
  );
  await sleep(1500);
}
  ...
  try {
    ...
    for (const campus of campuses) {
      ...
      for (const building of buildingsRefetched) {
        ...
        try {
          await selectBuilding(page, buildingName);
